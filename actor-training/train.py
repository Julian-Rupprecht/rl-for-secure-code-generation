...
def prepare_model_and_tokenizer(
    ...
    special_tokens=["<vuln>", "<safe>", "<cwe-id>", "<code>"]
):
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    model = T5ForConditionalGeneration.from_pretrained(model_name)

    special_token_dict = {"additional_special_tokens": special_tokens}
    added_token_count = tokenizer.add_special_tokens(special_token_dict)

    if added_token_count > 0:
        print(f"Added {added_token_count} special tokens to tokenizer.")
...
def test(model, test_dl, device, checkpoint_path, tokenizer):
    ...
    inner_model = T5ForConditionalGeneration.from_pretrained("Salesforce/codet5-large")
    inner_model.load_state_dict(torch.load(model_load_path))
    model = Model(inner_model).to(device)
...
def test2(model, test_ds, device, checkpoint_path, tokenizer):
    import torch
    ...
    logger.info(" ******* Start Testing *******")
    logger.info(f" Num examples = {len(test_ds)}")

    i = 1
    predictions = []
    num_codebleu_samples = 0
    model.to(device)
    model.eval()
    for sample in test_ds:
        ...
        with torch.no_grad():
            ...
            if sample['target'] == 1:
                score = calc_codebleu(references=decoded_refs, predictions=decoded_preds, lang="cpp")
                test_bleu += score['codebleu']
                num_codebleu_samples += 1

            predictions.extend(decoded_preds)

            if i % 500 == 0:
                logger.info(f" Test step {i} of {len(test_ds)}")
                logger.info(f" Allocated GPU memory: {torch.cuda.memory_allocated() / (1024**3):.2f} GB")
                logger.info(f" Reserved GPU memory: {torch.cuda.memory_reserved() / (1024**3):.2f} GB")
            i += 1
        
    test_loss /= len(test_ds)
    test_bleu = test_bleu / num_codebleu_samples if num_codebleu_samples > 0 else 0.0

    logger.info(f" Test loss = {test_loss}")
    logger.info(f" Test CodeBleu score (on target==1) = {test_bleu}")
...
def main():
    ...
    train_ds = ReposVulDataset(train_data_path, tokenizer)
    eval_ds = ReposVulDataset(eval_data_path, tokenizer)
    test_ds = ReposVulDataset(test_data_path, tokenizer)

    item = train_ds.__getitem__(18)
    logger.info(f"Decoded input (with special tokens):\n{tokenizer.decode(item['input_ids'], skip_special_tokens=False)}")
    logger.info(f"Decoded label:\n{tokenizer.decode(item['labels'], skip_special_tokens=False)}")
    logger.info(f"Raw target label:\n{item['target']}")
    ...
    test2(model, test_ds, device, checkpoint_path, tokenizer)    
    return 0
