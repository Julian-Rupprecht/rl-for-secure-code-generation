...
from helper import SpocDatasetLineWise, custom_collate_fn
...
def train(model, train_dl, eval_dl, num_epochs, device, checkpoint_path, tokenizer):
    ...
    for epoch in range(1, num_epochs+1):
        ...
        if abs(eval_loss - best_loss) < 0.001 or abs(eval_bleu - best_bleu) < 0.001:
            stop_patience -= 1
...
def main():
    ...
    train_ds = SpocDatasetLineWise(train_data_path, tokenizer)
    eval_ds = SpocDatasetLineWise(eval_data_path, tokenizer)
    test_ds = SpocDatasetLineWise(test_data_path, tokenizer)
